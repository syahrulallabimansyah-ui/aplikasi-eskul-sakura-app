...
switch ($action) {
    ...
    case 'admin_list_exams':
        ...
        $rows = $db->query("
            SELECT e.*, 
                   ...
                   (CASE WHEN e.token_expires_at IS NOT NULL AND e.token_expires_at <= NOW() THEN 1 ELSE 0 END) AS token_expired
    ...
    case 'admin_create_exam':
        ...
        $tokenExpiresAt = parseTokenExpiry($_POST['token_expires_at'] ?? '');

        if (empty($title)) jres(['success' => false, 'message' => 'Judul ujian wajib diisi.']);

        $stmt = $db->prepare("INSERT INTO exams (title, description, token, token_expires_at, duration_minutes, status, created_by) VALUES (?, ?, ?, ?, ?, 'draft', ?)");
        $stmt->execute([$title, $desc, $token, $tokenExpiresAt, $duration, $userId]);
    ...
    case 'admin_update_exam':
        ...
        $stmt = $db->prepare("UPDATE exams SET title=?, description=?, duration_minutes=?, token_expires_at=? WHERE id=?");
        $stmt->execute([$title, $desc, $duration, $tokenExpiresAt, $examId]);
    ...
    case 'admin_regenerate_token':
        ...
        $tokenExpiresAt = parseTokenExpiry($_POST['token_expires_at'] ?? '');
        $stmt = $db->prepare("UPDATE exams SET token=?, token_expires_at=? WHERE id=?");
        $stmt->execute([$newToken, $tokenExpiresAt, $examId]);
        jres(['success' => true, 'token' => $newToken, 'token_expires_at' => $tokenExpiresAt]);
        break;
    ...
    case 'user_verify_token':
        ...
        // Token kadaluarsa hanya menghalangi yang belum memulai ujian.
        // User yang sudah in_progress tetap bisa melanjutkan walau token sudah lewat batas waktu.
        if ((!$attempt || $attempt['status'] === 'not_started')
            && $exam['token_expires_at'] !== null
            && strtotime($exam['token_expires_at']) <= time()) {
            jres(['success' => false, 'message' => 'Token ujian ini sudah kadaluarsa. Hubungi admin untuk mendapatkan token baru.', 'token_expired' => true]);
        }
    ...
    case 'user_start_exam':
        ...
        if ((!$attempt || $attempt['status'] === 'not_started')
            && $exam['token_expires_at'] !== null
            && strtotime($exam['token_expires_at']) <= time()) {
            jres(['success' => false, 'message' => 'Token ujian ini sudah kadaluarsa.', 'token_expired' => true]);
        }
...
// =================================================================
// Helper: Parsing input tanggal/jam batas waktu token dari form
// (menerima format datetime-local "YYYY-MM-DDTHH:MM" atau kosong)
// =================================================================
function parseTokenExpiry(string $raw): ?string {
    $raw = trim($raw);
    if ($raw === '') return null;
