...
if ($_SERVER['REQUEST_METHOD'] === 'GET' && isset($_GET['action'])) {
    ...
    if ($_GET['action'] === 'get_lessons') {
        ...
        $stmt = $db->prepare("
            SELECT gl.id, gl.judul, gl.deskripsi, gl.urutan,
                   COUNT(gm.id) AS total_cerita
            FROM reading_lesson gl
            LEFT JOIN reading_story gm ON gm.lesson_id = gl.id AND gm.is_active = 1
    ...
    if ($_GET['action'] === 'get_ceritas') {
        ...
        $stmt = $db->prepare("
            SELECT id, judul, kategori, penjelasan_singkat, urutan
            FROM reading_story
    ...
    if ($_GET['action'] === 'get_cerita_detail') {
        $ceritaId = (int)($_GET['cerita_id'] ?? 0);
        $stmt = $db->prepare("
            SELECT gm.*, gl.judul AS lesson_judul, lv.kode AS level_kode
            FROM reading_story gm
            JOIN reading_lesson gl ON gl.id = gm.lesson_id
            JOIN grammar_level lv ON lv.id = gl.level_id
            WHERE gm.id = ? AND gm.is_active = 1
        ");
        $stmt->execute([$ceritaId]);
        $cerita = $stmt->fetch();
        if (!$cerita) {
            echo json_encode(['ok' => false, 'error' => 'Cerita tidak ditemukan']);
            exit;
        }
        echo json_encode(['ok' => true, 'cerita' => $cerita], JSON_UNESCAPED_UNICODE);
        exit;
    }
}
...
<head>
  ...
  <style>
    ...
    .lesson-card.open .cerita-list { display: block; }
    .cerita-loading {
      padding: 14px 16px;
      font-size: .85rem;
      color: var(--text-muted,#888);
    ...
    .cerita-item:last-child { border-bottom: none; }
    .cerita-item:hover { background: rgba(183,75,75,.05); }
    .cerita-dot {
      flex-shrink: 0;
    ...
    .cerita-info { flex: 1; min-width: 0; }
    .cerita-title {
    ...
    .cerita-modal-overlay.open { display: flex; }
    .cerita-modal {
      background: var(--card-bg,#fff);
    ...
    .cerita-modal-breadcrumb {
      font-size: .73rem; color: var(--text-muted,#888);
      margin-bottom: 8px;
      display: flex; align-items: center; gap: 4px;
      flex-wrap: wrap;
    }
    .cerita-modal-breadcrumb span { color: var(--torii,#b74b4b); font-weight: 700; }
    .cerita-modal-title {
    ...
    .cerita-modal-konten p { margin: 0 0 14px; }
    .cerita-modal-konten p:last-child { margin-bottom: 0; }
    .cerita-modal-konten-empty {
      text-align: center;
      padding: 30px 0;
      color: var(--text-muted,#bbb);
      font-size: .88rem;
    ...
    .cerita-modal-close:hover { background: rgba(0,0,0,.15); }
...
<body class="dashboard-page">
  ...
  <main class="dashboard-main">
    ...
    <section class="tb-page-header fade-up">
      <div class="tb-page-kanji">読書</div>
      <div class="tb-page-title">Membaca Jepang</div>
      <div class="tb-page-sub">Pilih level JLPT lalu baca cerita, artikel & berita sesuai kemampuanmu</div>
    </section>
    ...
    <div class="tb-content">
      ...
      <div class="lessons-container fade-up" id="lessonsContainer">
        <div class="cerita-loading">⏳ Memuat bab...</div>
      </div>
  ...
  <!-- Cerita Detail Modal -->
  <div class="cerita-modal-overlay" id="ceritaModal" onclick="closeCeritaModal(event)">
    <div class="cerita-modal" id="ceritaModalContent" style="position:relative;">
      <button class="cerita-modal-close" onclick="closeCeritaModal(null, true)">✕</button>
      <div class="cerita-modal-handle"></div>
      <div id="ceritaModalInner"></div>
    </div>
  </div>
  ...
  <script>
    ...
    async function loadLessons(levelId) {
      ...
      container.innerHTML = `
        <div style="padding:40px 0; text-align:center; color:var(--text-muted,#888);">
          ...
          <div style="font-size:.85rem;">Memuat bab...</div>
        </div>`;

      try {
        const res  = await fetch(`membaca.php?action=get_lessons&level_id=${levelId}`);
        ...
        if (!data.ok || !data.lessons.length) {
          container.innerHTML = `
            <div class="empty-state">
              ...
              <div class="empty-state-text">Belum ada bab untuk level ini.<br>Admin akan segera menambahkan konten.</div>
            </div>`;
          return;
        }
          ...
          <div class="lesson-card" id="lesson-${lesson.id}" onclick="toggleLesson(this, ${lesson.id})">
            <div class="lesson-card-header">
              ...
              <div class="lesson-meta">
                <span>${lesson.total_cerita} cerita</span>
              </div>
              <div class="lesson-arrow">›</div>
            </div>
            <div class="cerita-list" id="cerita-list-${lesson.id}">
              <div class="cerita-loading">Memuat...</div>
            </div>
          </div>
        `).join('');
    ...
    async function toggleLesson(card, lessonId) {
      ...
      const listEl = document.getElementById(`cerita-list-${lessonId}`);

      // Jika sudah pernah dimuat, jangan muat ulang
      if (listEl.dataset.loaded) return;

      try {
        const res  = await fetch(`membaca.php?action=get_ceritas&lesson_id=${lessonId}`);
        const data = await res.json();
        listEl.dataset.loaded = '1';

        if (!data.ok || !data.ceritas.length) {
          listEl.innerHTML = `<div class="cerita-loading" style="color:var(--text-muted,#aaa);">Belum ada cerita di bab ini.</div>`;
          return;
        }

        listEl.innerHTML = data.ceritas.map(m => `
          <div class="cerita-item" onclick="event.stopPropagation(); openCerita(${m.id})">
            <div class="cerita-dot"></div>
            <div class="cerita-info">
              <div class="cerita-title">${escHtml(m.judul)} <span class="kategori-badge">${escHtml(m.kategori || 'Cerita')}</span></div>
              ${m.penjelasan_singkat ? `<div class="cerita-desc">${escHtml(m.penjelasan_singkat)}</div>` : ''}
            </div>
            <div class="cerita-arrow">›</div>
          </div>
        `).join('');

      } catch(e) {
        listEl.innerHTML = `<div class="cerita-loading" style="color:var(--torii,#b74b4b);">Gagal memuat cerita.</div>`;
      }
    }

    // ── Open Cerita Detail Modal ──
    async function openCerita(ceritaId) {
      const modal   = document.getElementById('ceritaModal');
      const inner   = document.getElementById('ceritaModalInner');
      ...
      try {
        const res  = await fetch(`membaca.php?action=get_cerita_detail&cerita_id=${ceritaId}`);
        const data = await res.json();

        if (!data.ok) {
          inner.innerHTML = `<div style="text-align:center;padding:30px 0;color:var(--torii,#b74b4b);">⚠️ Cerita tidak ditemukan.</div>`;
          return;
        }

        const m = data.cerita;
        inner.innerHTML = `
          <div class="cerita-modal-breadcrumb">
            <span>${escHtml(m.level_kode)}</span> ›
            <span>${escHtml(m.lesson_judul)}</span> ›
            <span>${escHtml(m.kategori || 'Cerita')}</span>
          </div>
          <div class="cerita-modal-title">${escHtml(m.judul)}</div>
          ${m.penjelasan_singkat
            ? `<div class="cerita-modal-short">${escHtml(m.penjelasan_singkat)}</div>`
            : ''}
          <div class="cerita-modal-konten">
            ${m.konten
              ? m.konten  /* konten bisa berisi HTML dari admin */
              : `<div class="cerita-modal-konten-empty">📝 Konten cerita sedang disiapkan oleh admin.</div>`}
          </div>`;

      } catch(e) {
        inner.innerHTML = `<div style="text-align:center;padding:30px 0;color:var(--torii,#b74b4b);">⚠️ Gagal memuat cerita.</div>`;
      }
    }

    function closeCeritaModal(event, force = false) {
      if (force || (event && event.target === document.getElementById('ceritaModal'))) {
        document.getElementById('ceritaModal').classList.remove('open');
    ...
    document.addEventListener('keydown', e => {
      if (e.key === 'Escape') closeCeritaModal(null, true);
    });
  </script>
</body>
</html>
